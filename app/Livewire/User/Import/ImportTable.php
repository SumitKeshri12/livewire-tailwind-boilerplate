...
final class ImportTable extends PowerGridComponent
{
    ...
    public string $tableName = 'user_import_table';
    ...
    public function datasource(): Builder
    {
        ...
        return ImportLog::query()->where('model_name', config('constants.import_type.user'));
    }
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('created_at_formatted', fn (ImportLog $model) => Carbon::parse($model->created_at)->format(config('constants.default_datetime_format')));
    }

    public function columns(): array
    {
        return [
            Column::make(__('messages.users.import.table.file'), 'file_name')
                ->searchable()
                ->sortable(false),

            Column::make(__('messages.users.import.table.date'), 'created_at_formatted', 'created_at')
                ->sortable(false),

            Column::make(__('messages.users.import.table.no_of_rows'), 'no_of_rows')
                ->sortable(false),

            Column::make(__('messages.users.import.table.status'), 'status')
                ->sortable(false),

            Column::action(__('messages.users.import.table.actions')),
        ];
    }
    ...
    public function actions(ImportLog $row): array
    {
        ...
        if ($row->status == config('constants.import_csv_log.status.key.fail')) {
            $actions[] = Button::add('view_log')
                ...
                ->dispatchTo('user.import.import-error-page', 'viewImportErrors', ['errorLogs' => $row->error_log]);
        }
