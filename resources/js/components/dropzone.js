...
class DropzoneComponent {
    ...
    init() {
        if (typeof Dropzone === 'undefined') {
            console.log('Dropzone library not loaded yet, retrying...');
        ...
        if (!dropzoneContainer) {
            console.error('Dropzone container not found:', this.config.dropzoneContainer);
            return;
        }

        // Check if URL is provided
        if (!this.config.url) {
            console.error('Dropzone: No URL provided in configuration');
            return;
        }

        console.log('Initializing Dropzone with config:', this.config);
        ...
        if (this.dropzone) {
            ...
            setTimeout(() => {
                if (this.dropzone && !this.dropzone._eventHandlersSet) {
                    console.log('Setting up event handlers as fallback');
                    this.setupEventHandlers();
                }
            }, 200);
        } else {
            console.error('Failed to initialize Dropzone');
        }
    }
    ...
    createDropzoneInstance() {
        ...
        this.dropzone = new Dropzone(this.config.container, {
            ...
            init: () => {
                console.log('Dropzone init callback called');
            ...
            console.log('Dropzone instance created:', this.dropzone);
        } catch (error) {
            console.error('Error creating Dropzone instance:', error);
            this.dropzone = null;
        }
    }
    ...
    setupEventHandlers() {
        if (!this.dropzone) {
            console.error('Dropzone instance not available for event handlers');
            return;
        }

        console.log('Setting up event handlers for dropzone');
