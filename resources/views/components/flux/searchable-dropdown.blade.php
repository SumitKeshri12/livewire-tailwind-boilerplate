@props([
    ...
    'disabled' => false,
    'required' => false,
    'valueColumn' => null, // Custom column name for value (e.g., 'email_id', 'fname', 'id')
    'labelColumn' => null, // Custom column name for label (e.g., 'email', 'full_name', 'name')
])
...
@php
    ...
    if (!empty($options)) {
        ...
        // Helper function to get value from option based on valueColumn
        $getValue = function($option) use ($valueColumn) {
            if ($valueColumn) {
                // Use custom value column
                if (is_object($option)) {
                    return $option->{$valueColumn} ?? null;
        ...
        // Helper function to get label from option based on labelColumn
        $getLabel = function($option) use ($labelColumn) {
            if ($labelColumn) {
                // Use custom label column
                if (is_object($option)) {
                    return $option->{$labelColumn} ?? null;
                } elseif (is_array($option)) {
                    return $option[$labelColumn] ?? null;
                }
            } else {
