<div class="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col" id="kt_content">
    ...
    <div x-data="{ openModule: null }" class="space-y-4">
        ...
        <div class="border border-gray-200 dark:border-gray-700 rounded-lg">
            <!-- Header -->
            <button type="button"
                @click="openModule = (openModule === {{ $permission['id'] }} ? null : {{ $permission['id'] }})"
                class="w-full flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-t-lg">
                <span class="font-semibold text-gray-900 dark:text-white">{{ $permission['label'] }}</span>
                <svg x-show="openModule !== {{ $permission['id'] }}" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
            ...
            <!-- Sub Permissions -->
            <div x-show="openModule === {{ $permission['id'] }}" x-transition
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div
                        ...
                        <input id="perm-{{ $permission['id'] }}-{{ $subPermission['id'] }}" type="checkbox"
                            class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                            value="{{ $subPermission['id'] }}" @checked($subPermission['is_permission'])
                            wire:change="setUnsetPermission({{ $subPermission['id'] }}, $event.target.checked, {{ $permission['id'] }})">
                        <label for="perm-{{ $permission['id'] }}-{{ $subPermission['id'] }}"
