<div class="flex flex-col gap-6">
    ...
    <form method="POST" wire:submit="login" class="flex flex-col gap-6">
        ...
        <flux:input wire:model="email" :label="__('messages.login.label_email')" type="email" required autofocus
            autocomplete="email" placeholder="email@example.com" data-testid="email" id="email" />

        <!-- Password -->
        <div class="relative">
            <flux:input wire:model="password" :label="__('messages.login.label_password')" type="password" required
                autocomplete="current-password" :placeholder="__('messages.login.label_password')" viewable
    ...
    @if (Route::has('password.request'))
        <div class="space-x-1 text-sm text-left rtl:space-x-reverse text-zinc-600 dark:text-zinc-400">
            <flux:link data-testid="forgot_password" :href="route('password.request')" wire:navigate>
                {{ __('messages.login.forgot_password_title') }}?</flux:link>
        </div>
    @endif
...
@push('scripts')
    <script nonce="{{ $nonce }}"
        src="https://www.google.com/recaptcha/api.js?render={{ config('constants.google_recaptcha_key') }}"></script>
    <script nonce="{{ $nonce }}">
